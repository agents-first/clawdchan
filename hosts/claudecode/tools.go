	...
	return func(ctx context.Context, _ mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		return jsonResult(map[string]any{
			...
			"model": []string{
				...
				"Session start context is in startup_digest. If unread_total > 0, surface startup_digest.summary before normal task flow.",
...
func buildStartupDigest(ctx context.Context, n *node.Node) map[string]any {
	...
	me := n.Identity()
	unreadTotal, pendingTotal := 0, 0
	for _, t := range threads {
		envs, err := n.ListEnvelopes(ctx, t.ID, 0)
		if err != nil {
			continue
		}
		unread, pending, lastActivity := unreadStatsForThread(envs, me)
	...
	summary := startupDigestSummary(unreadTotal, pendingTotal)
	return map[string]any{
		"generated_at_ms":       time.Now().UnixMilli(),
		"unread_total":          unreadTotal,
		"pending_asks_total":    pendingTotal,
...
// Unread means inbound envelopes newer than the latest local outbound envelope.
// Any unanswered ask_human remains unread regardless of timestamp.
func unreadStatsForThread(envs []envelope.Envelope, me identity.NodeID) (unread int, pending int, lastActivity int64) {
